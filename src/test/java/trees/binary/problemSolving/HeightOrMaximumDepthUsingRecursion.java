...
public class HeightOrMaximumDepthUsingRecursion {

    public Integer getHeightUsingRecursion(BinaryTreeNode node) {
        if (null == node) return 0;

        //using math function :
        return Math.max(getHeightUsingRecursion(node.getLeft()), getHeightUsingRecursion(node.getRight())) + 1;

//        //Using normal comparison operator:
//        Integer leftSubtreeHeight = getHeightUsingRecursion(node.getLeft());
//        Integer rightSubtreeHeight = getHeightUsingRecursion(node.getRight());
//
...
//        rootRight.setRight(rootRightRight);
        ...
        Integer result = new HeightOrMaximumDepthUsingRecursion().getHeightUsingRecursion(root);
